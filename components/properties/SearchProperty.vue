...
<template>
  <div class="box-input-main-container">
    ...
    >
      <SortInput v-model="selectedLocation" :input-image="'/icons/Location.svg'" label="Location" :options="locationOptions" />
      <SortInput v-model="selectedPropertyType" :input-image="'/icons/HouseIcon.svg'" label="Property Type" :options="propertyTypeOptions" />
      <SortInput v-model="selectedPrice" :input-image="'/icons/CamIcon.svg'" label="Pricing Range" :options="priceOptions" />
      <SortInput v-model="selectedAreaSqFt" :input-image="'/icons/CubeIc.svg'" label="Property Size" :options="areaSqFtOptions" />
      <SortInput v-model="selectedBuildYear" :input-image="'/icons/Calendar.svg'" label="Build Year" :options="buildYearOptions" />
    </div>
  ...
  <div class="my-14">
    ...
    <div v-if="showResults && filteredProperties.length > 0" class="px-4 text-left text-white">
      <div class="flex w-[80%] justify-between items-center max-w-auto mx-auto">
        ...
        <button class="font-sans text-gray-300 text-3xl text-center" @click="handleClearResults()">Clear results</button>
      </div>
      <ul class="space-y-6">
        <li
        ...
        >
          ...
          <div class="space-y-2 text-lg text-[#e6e0e0] font-sans">
            ...
            <p><strong>Size:</strong> {{ property.areaSqFt }} sq.ft</p>
            <p><strong>Year Built:</strong> {{ property.buildYear }}</p>
          </div>
        </li>
      </ul>
    </div>

    <p v-else-if="showResults && !loading && !error" class="text-center text-gray-500">
